...
export const TopTableSection = ({ 
    campaignData, adGroupData, productAdData, targetingData, searchTermData, currency, isReportMode = false 
  ...
  const getTableDataForReport = (reportType: string, metric: string, tabName: string) => {
      ...
      // Use the 'metric' argument directly
      const sortKey = keyMap[metric] || 'sales';

      const sorted = [...source].sort((a, b) => {
          const valA = a[sortKey] || 0;
          const valB = b[sortKey] || 0;
          // Use the 'reportType' argument directly
  ...
  // Define currentReport here so it is available for the return statement
  const currentReport = reports.find(r => r.id === activeReportId) || reports[0];

  // Memoize active data for Dashboard View
  const activeTableData = useMemo(() => {
      const r = reports.find(r => r.id === activeReportId) || reports[0];
      return getTableDataForReport(r.type, r.metric, activeTab);
  }, [activeTab, campaignData, adGroupData, productAdData, targetingData, searchTermData, activeReportId, reports]);
  ...
  // ... (Same as your original code, hidden in Report Mode via UI logic)
  const handleAddReport = () => { if(reports.length>=10)return; const r={id:Date.now().toString(),type:newType,metric:newMetric,isEditing:true}; setReports([...reports,r]); setActiveReportId(r.id); setIsAddingNew(false); };
  ...
  const EditForm = ({ reportId }: { reportId: string }) => (
    <div className="flex flex-col gap-2" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex gap-2 justify-end"><button onClick={(e)=>{e.stopPropagation();setReports(reports.map(r=>r.id===reportId?{...r,isEditing:false}:r))}} className="w-6 h-6 flex items-center justify-center bg-gray-100 rounded hover:bg-gray-200 text-gray-600"><X size={14}/></button><button onClick={(e)=>handleSaveEdit(e,reportId)} className="w-6 h-6 flex items-center justify-center bg-[#4aaada] rounded hover:bg-[#3a9aca] text-white"><Check size={14}/></button></div>
    </div>
  );
  ...
  const TableInstance = ({ reportType, metric, tabName }: { reportType: string, metric: string, tabName: string }) => {
      ...
      // Pass the specific tabName explicitly, overriding state activeTab
                ...
                <table className="w-full border-separate border-spacing-0">
                    <thead className="bg-[#f8f9fa] z-20">
                        <tr>
                            ...
                            <th className="sticky left-[60px] top-0 z-30 bg-[#f8f9fa] border-b border-r border-[#e2e2e2] px-4 py-3 text-left text-xs font-semibold text-gray-500 min-w-[320px]">{activeTab}</th>
                        ...
                        {data.length === 0 ? <tr><td colSpan={12} className="text-center py-10 text-gray-400">No data found</td></tr> : 
                            data.map((row, index) => (
                                <tr key={index} className="hover:bg-gray-50 group">
                                    ...
                                    <td className="border-b border-r border-[#e2e2e2] px-4 py-3 align-top">{renderEntityInfo(row, activeTab)}</td>
  ...
  return (
    <div className={`bg-white rounded-lg ${isReportMode ? '' : 'border border-[#e2e2e2] flex flex-col mb-10 overflow-hidden h-[650px]'}`}>
      ...
      <div className={`flex flex-1 ${isReportMode ? 'flex-col' : 'border-t border-[#e2e2e2] overflow-hidden'}`}>
        ...
        {!isReportMode && (
            <div className="w-[280px] flex-shrink-0 border-r border-[#e2e2e2] flex flex-col bg-white">
                <div className="flex-1 overflow-y-auto p-4 space-y-2">
                    {reports.map((report) => (
                        <div key={report.id} onClick={() => !report.isEditing && setActiveReportId(report.id)} className={`relative transition-all cursor-pointer py-3 px-3 border-l-4 ${activeReportId === report.id ? 'border-[#4aaada] bg-[#f8f9fa]' : 'border-transparent hover:bg-gray-50'}`}>
        ...
        {isReportMode ? (
            // REPORT MODE: Loop through TABS
            <div className="flex flex-col gap-6">
                {TABS.filter(t => t !== 'Goals').map(tabName => (
                    <TableInstance 
                        key={tabName} 
                        reportType="Top 10" 
                        metric="Ad Sales" 
            ...
            // DASHBOARD MODE: Pass current report state
            <TableInstance 
