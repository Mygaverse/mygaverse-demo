...
const METRIC_HEADERS = [
    'Ad Sales', 'Ad Spend', 'ACOS', 'ROAS', 'Ad Orders', 'CVR', 'Impressions', 'Clicks', 'CTR', 'CPC'
];

export const TopTableSection = ({ 
    campaignData, adGroupData, productAdData, targetingData, searchTermData, currency 
}: TopTableSectionProps) => {
  const [activeTab, setActiveTab] = useState('Campaigns');
  const [sidebarDesign, setSidebarDesign] = useState<'Old' | 'New'>('Old');
  
  // Reports State
  const [reports, setReports] = useState<Report[]>([
    { id: '1', type: 'Top 5', metric: 'Ad Sales', isEditing: false },
  ]);
  ...
  const tableData = useMemo(() => {
      ...
      switch (activeTab) {
          ...
          case 'Goals': source = []; break; // Goals not fully wired yet
      }
      ...
      const sortKey = keyMap[currentReport.metric] || 'sales';

      // 3. Sort
      const sorted = [...source].sort((a, b) => {
          const valA = a[sortKey] || 0;
          const valB = b[sortKey] || 0;
          return currentReport.type.includes('Top') ? valB - valA : valA - valB;
      });

      // 4. Slice
      const limit = currentReport.type.includes('10') ? 10 : 5;
      const sliced = sorted.slice(0, limit);

      // 5. Rank
      return sliced.map((item, idx) => ({ ...item, rank: idx + 1 }));

  }, [activeTab, campaignData, adGroupData, productAdData, targetingData, searchTermData, currentReport]);


  // --- Render Helpers (Dynamic Column 2) ---
  const renderEntityInfo = (row: any) => {
    // 1. Product Ads (Image + Title + ASIN/SKU)
    if (activeTab === 'Product Ads') {
        return (
            <div className="flex items-start gap-3">
                <img 
                    src={row.productImage || "https://placehold.co/40x40?text=Prod"} 
                    alt="product" 
                    className="w-10 h-10 rounded border border-gray-200 object-cover" 
                />
                <div className="flex flex-col gap-1">
                    <a href="#" className="text-sm text-[#0066b7] hover:underline font-medium truncate max-w-[240px]" title={row.productName}>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                        <Badge variant="flag" iconStr={`https://flagcdn.com/w20/${row.flag || 'us'}.png`}>{row.storeName || 'Store'}</Badge>
    ...
    if (activeTab === 'Targeting' || activeTab === 'Search Terms') {
        ...
        return (
            <div className="flex flex-col gap-1.5">
                <div className="flex items-center gap-2">
                    ...
                    {activeTab === 'Search Terms' && <Files size={10} className="text-gray-400"/>}
                </div>
    ...
    // 3. Default (Campaigns / Ad Groups)
    return (
        <div className="flex flex-col gap-1.5">
            <a href="#" className="text-sm text-[#0066b7] hover:underline font-medium truncate max-w-[280px]" title={row.name}>{row.name || 'Unknown'}</a>
            <div className="flex items-center gap-2 flex-wrap">
                <Badge variant="flag" iconStr={`https://flagcdn.com/w20/${row.flag || 'us'}.png`}>{row.storeName || 'Store'}</Badge>
                {activeTab === 'Ad Groups' && <Badge variant="neutral" className="bg-gray-100 text-gray-500 max-w-[100px] truncate">{row.campaignName}</Badge>}
                {activeTab === 'Campaigns' && <Badge variant="neutral">{row.type || 'SP'}</Badge>}
                <Badge variant={row.enabled ? 'status-enabled' : 'status-paused'}>{row.enabled ? 'Enabled' : 'Paused'}</Badge>
            </div>
        </div>
    );
  ...
  // ... (Handlers & EditForm same as previous) ...
  ...
  const handleRemoveReport = (e:any,id:string)=>{e.stopPropagation();if(reports.length<=1)return;const f=reports.filter(r=>r.id!==id);setReports(f);if(id===activeReportId&&f.length>0)setActiveReportId(f[0].id);};
  const handleEditClick = (e:any,r:Report)=>{e.stopPropagation();setEditType(r.type);setEditMetric(r.metric);setReports(reports.map(rep=>rep.id===r.id?{...rep,isEditing:true}:{...rep,isEditing:false}));setActiveReportId(r.id);};
  const handleSaveEdit = (e:any,id:string)=>{e.stopPropagation();setReports(reports.map(r=>r.id===id?{...r,type:editType,metric:editMetric,isEditing:false}:r));};

  const EditForm = ({ reportId }: { reportId: string }) => (
    <div className="flex flex-col gap-2" onClick={e => e.stopPropagation()}>
        <div className="flex gap-2">
            <select className="text-xs border border-gray-300 rounded px-2 py-1.5 w-full bg-white text-gray-700" value={editType} onChange={(e) => setEditType(e.target.value)}>{REPORT_TYPES.map(t=><option key={t} value={t}>{t}</option>)}</select>
            <select className="text-xs border border-gray-300 rounded px-2 py-1.5 w-full bg-white text-gray-700" value={editMetric} onChange={(e) => setEditMetric(e.target.value)}>{REPORT_METRICS.map(m=><option key={m} value={m}>{m}</option>)}</select>
        </div>
        <div className="flex gap-2 justify-end">
            <button onClick={(e)=>{e.stopPropagation();setReports(reports.map(r=>r.id===reportId?{...r,isEditing:false}:r))}} className="w-6 h-6 flex items-center justify-center bg-gray-100 rounded hover:bg-gray-200 text-gray-600"><X size={14}/></button>
  ...
  return (
    <div className="bg-white rounded-lg border border-[#e2e2e2] flex flex-col mb-10 overflow-hidden h-[650px]">
      
      {/* 1. Header Section with Tabs and Plus Button */}
      <div className="p-6 pb-0">
        <div className="flex items-center gap-3 mb-6 w-full">
            ...
            <div className="relative flex-shrink-0"><Button variant="secondaryIcon" size="icon"><PlusCircleFill className="text-[#4aaada]" size={16} /></Button></div>
        </div>
      </div>

      {/* 2. Main Content Area */}
      <div className="flex flex-1 border-t border-[#e2e2e2] overflow-hidden">
        
        {/* Left Sidebar (Report Manager) */}
        <div className="w-[280px] flex-shrink-0 border-r border-[#e2e2e2] flex flex-col bg-white">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2">
                {reports.map((report) => (
                    <div 
                        key={report.id} 
                        onClick={() => !report.isEditing && setActiveReportId(report.id)} 
                        className={`
                            relative transition-all cursor-pointer 
                            py-3 px-3 border-l-4 
                            ${activeReportId === report.id ? 'border-[#4aaada] bg-[#f8f9fa]' : 'border-transparent hover:bg-gray-50'}`}>
                        
                        {/* Common Logic for both: If editing, show form. If not, show display */}
                        {report.isEditing ? 
                            <EditForm reportId={report.id} /> : (
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-white p-6">
            <div className="flex justify-between items-center mb-4">
                <div><h3 className="text-base font-bold text-gray-800">{currentReport.type} {activeTab} by {currentReport.metric}</h3><p className="text-xs text-gray-400 mt-1">Data Range Applied</p></div>
                <Button variant="secondary" icon={<LayoutThreeColumns size={16} />}>Columns</Button>
            </div>
                ...
                <table className="w-full border-separate border-spacing-0">
                    <thead className="bg-[#f8f9fa] z-20">
                        <tr>
                            ...
                            <th className="sticky left-[60px] top-0 z-30 bg-[#f8f9fa] border-b border-r border-[#e2e2e2] px-4 py-3 text-left text-xs font-semibold text-gray-500 min-w-[320px] shadow-sm">{activeTab} <ChevronDown className="inline ml-1" size={10}/></th>
                    ...
                    <tbody className="bg-white">
                        {tableData.length === 0 ? <tr><td colSpan={12} className="text-center py-20 text-gray-400">No {activeTab.toLowerCase()} data found</td></tr> : 
                            tableData.map((row, index) => (
                                <tr key={index} className="hover:bg-gray-50 group">
                                    <td className="sticky left-0 z-10 bg-white group-hover:bg-gray-50 border-b border-r border-[#e2e2e2] px-4 py-3 text-sm text-gray-900 font-medium text-center">{row.rank}</td>
                                    <td className="sticky left-[60px] z-10 bg-white group-hover:bg-gray-50 border-b border-r border-[#e2e2e2] px-4 py-3 shadow-sm align-top">{renderEntityInfo(row)}</td>
