...
export interface TabData { id: string; metric1: string; metric2: string; }
...
interface PerformanceOverTimeProps {
    ...
    // Lifted State
    tabs: TabData[];
    activeTabId?: string; // Optional for Report Mode (shows all)
...
export const PerformanceOverTime = ({
    ...
    const handleAddConfirm = () => {
        ...
        if (customTabId) {
            onTabsChange(tabs.map(t => t.id === customTabId ? newTab : t));
        } else {
            onTabsChange([...tabs, newTab]);
            setCustomTabId(newId);
        }
        onTabChange(newId);
        setIsAddOpen(false);
    };
    ...
    const handleEditConfirm = () => {
        ...
        onTabsChange(tabs.map(t => t.id === activeTabId ? { ...t, metric1: editComp1, metric2: editComp2 } : t));
        setIsEditOpen(false);
    };
                ...
                {!isReportMode && (
                    <div className="flex items-center gap-3 mb-6 w-full">
                        <div className="flex-1 flex items-center border border-[#e2e2e2] rounded-md overflow-hidden h-[42px]">
                            {tabs.map((tab) => (
                                <button key={tab.id} onClick={() => onTabChange(tab.id)} className={`flex-1 h-full text-sm text-center relative transition-colors font-medium border-r border-[#e2e2e2] last:border-0 flex items-center justify-center gap-2 px-2 ${activeTabId === tab.id ? 'text-[#212529] bg-gray-50' : 'text-gray-400 hover:text-gray-600 hover:bg-gray-50/50'}`}>
