...
import { BQoolUniversityButton } from "@/components/bqool/im/BQoolUniversityButton";
// Modals
...
import { Loader } from '@/components/bqool/ui/Loader';
import { BQoolInsightWidget } from "./insights/BQoolInsightWidget";
...
export function V2DashboardContent() {
  ...
  const [exchangeRates, setExchangeRates] = useState<Record<string, number>>({}); // Store rates
  ...
  // --- LIFTED STATE FOR REPORTS & CHARTS ---
  // Reports
  ...
  const [chartTabs, setChartTabs] = useState<TabData[]>([
    ...
    { id: 'fixed-2', metric1: 'Impressions', metric2: 'Clicks' },
    { id: 'fixed-3', metric1: 'Ad Orders', metric2: 'CVR' }
  ]);
  ...
  // --- 2.1 FETCH RATES ---
  useEffect(() => {
    fetch('https://api.exchangerate-api.com/v4/latest/USD') // Free public API
      .then(res => res.json())
      .then(data => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const [campSnap, agSnap, padSnap, targSnap, stSnap, goalSnap] = await Promise.all([
          ...
          getDocs(query(collection(db, 'goals'), where('storeId', 'in', stores))),
        ]);
        ...
        setRawGoals(goalSnap.docs.map(d => ({ id: d.id, ...d.data() })));
  ...
  const dashboardData = useMemo(() => {
    ...
    const metricFactor = factor * rate;

    const processMetrics = (data: any[]) => {
      return data.map(item => {
        const sales = (item.sales || 0) * metricFactor;
        const spend = (item.spend || 0) * metricFactor;
    ...
    const procGoals = processMetrics(filterData(rawGoals));
  ...
  }, [rawCampaigns, rawAdGroups, rawProductAds, rawTargeting, rawSearchTerms, rawGoals, adType, dateRange, currentSelection, selectedCurrency, exchangeRates]);
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-[1600px] mx-auto pb-10">
      <div className="flex items-center justify-between">
        <Breadcrumb />
        <BQoolUniversityButton variant="branding" size="md" />
      </div>
      ...
      <div className="flex flex-wrap items-center gap-4">
        <div className="flex items-center shadow-sm rounded-md h-[48px]">
          ...
          <SingleSelect label="Currency" value={selectedCurrency} options={['USD', 'CAD', 'EUR', 'GBP', 'JPY', 'View Rate']} onChange={handleCurrencyChange} width="w-[100px]" />
      ...
      */}
      ...
      <div className="flex flex-col gap-6">
        {loading ? (
          <Loader text="Loading Dashboard Data..." className="z-[2000]" />
        ) : (
          /* Pass data to the extracted component */
          <>
            <BQoolInsightWidget
              campaigns={dashboardData.campaigns}
              adGroups={dashboardData.adGroups}
              products={dashboardData.productAds}
              targeting={dashboardData.targeting}
              searchTerms={dashboardData.searchTerms}
              stats={dashboardData.chartData}
            />
            <DashboardWidgets
              data={dashboardData}
            ...
            <div className="flex justify-center pt-4">
              <Button variant="secondary" size="lg" onClick={() => setIsReportOpen(true)} icon={<Eye size={16} />} hideTextOnMobile={true}>Preview Report</Button>
            </div>
          </>
        )}
      </div>
