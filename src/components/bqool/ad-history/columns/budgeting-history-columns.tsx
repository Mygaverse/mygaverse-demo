import React from 'react';
import { ColumnDef } from "@/components/bqool/tables/DynamicTable";
import { EntityCell } from "@/components/bqool/tables/cells/EntityCell";
import { Badge } from "@/components/bqool/ui/Badge";

export interface BudgetingHistoryRow {
    id: string;
    dateTime: string;
    budgetingGroupName: string;
    goalName: string;
    goalType: string;
    campaignName: string;
    storeFlag: string;
    storeName: string;
    adType: string;
    ruleName: string;
    budgetFrom: string;
    budgetTo: string;
}

export const getBudgetingHistoryColumns = (): ColumnDef<BudgetingHistoryRow>[] => [
    { 
        key: 'dateTime', 
        header: 'Date & Time', 
        width: '160px', 
        render: (row) => <div className="text-sm font-medium text-gray-700 whitespace-normal">{row.dateTime}</div> 
    },
    { 
        key: 'group', 
        header: 'Budgeting Group', 
        width: '250px',
        render: (row) => (
            <span 
                className="text-sm text-[#0066b7] cursor-pointer hover:underline block truncate max-w-[230px]" 
                title={row.budgetingGroupName}
            >
                {row.budgetingGroupName}
            </span>
        )
    },
    { 
        key: 'goals', 
        header: 'Goals', 
        width: '200px',
        render: (row) => row.goalName !== '-' ? (
            <EntityCell 
                title={row.goalName} 
                badges={[<Badge key="t" variant="neutral" size="sm">{row.goalType}</Badge>]} 
            />
        ) : <span className="text-gray-400 text-sm">-</span>
    },
    { 
        key: 'campaigns', 
        header: 'Campaigns', 
        width: '220px',
        render: (row) => (
            <EntityCell 
                title={row.campaignName} 
                badges={[
                    <div key="s" className="flex items-center gap-1 border px-2 py-0.5 rounded border-[#e2e2e2] bg-gray-50">
                        <img src={`https://flagcdn.com/w20/${row.storeFlag || 'us'}.png`} className="w-3 h-2 rounded-[1px]" alt="flag"/>
                        <span className="text-[10px] text-gray-500">{row.storeName}</span>
                    </div>,
                    <Badge key="a" variant="neutral" size="sm">{row.adType}</Badge>
                ]} 
            />
        )
    },
    { 
        key: 'rule', 
        header: 'Budgeting Rule', 
        width: '140px', 
        align: 'center', 
        render: (row) => <Badge variant="auto-harvesting" size="sm">{row.ruleName}</Badge> 
    },
    { 
        key: 'from', 
        header: 'Budget changed from', 
        width: '140px', 
        align: 'right', 
        render: (row) => <span className="text-gray-500 text-sm mr-2">{row.budgetFrom}</span> 
    },
    { 
        key: 'to', 
        header: 'Budget changed to', 
        width: '140px', 
        align: 'right', 
        render: (row) => <span className="font-bold text-gray-900">{row.budgetTo}</span> 
    },
];