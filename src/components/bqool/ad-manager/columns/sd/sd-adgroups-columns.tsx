...
const SDMetricHeader = ({ label }: { label: string }) => (
    <div className="flex flex-col items-end leading-tight">
        <span>{label}</span>
        <span className="text-[9px] text-gray-400 font-normal whitespace-nowrap">(View & Clicks)</span>
    </div>
);
...
export const getSDAdGroupColumns = (
    ...
    // Spec Column Widths
    const WIDTH_FORMAT = '100px'; 
    const WIDTH_STRAT  = '140px';
    const WIDTH_TGT    = '120px';
    ...
    return [
                ...
                <div style={{ width: WIDTH_CAMP }}>
                    <EntityCell 
                        ...
                        badges={[
                            ...
                            <Badge key="b" variant="neutral" size="sm">SD</Badge>
                        ]} 
                    />
                </div>
            )
        ...
        // --- 6. DEFAULT BID ---
        { 
            key: 'defaultBid', header: 'Bids', align: 'center', width: '130px',
            render: (row) => (
                <BudgetCell 
                    value={row.defaultBid ?? 0} 
                    isAuto={false} 
                    onChange={(val) => onBidChange(row.id, val)}
                />
            )
        },
        ...
        {
            key: 'optimization', header: 'Optimization Strategy', width: WIDTH_STRAT, align: 'center',
            render: (row) => (
        ...
        {
            key: 'targeting', header: 'Targeting Type', width: WIDTH_TGT, align: 'center',
            render: (row) => (
                <span className="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-medium bg-gray-100 text-gray-700 border border-gray-200">
                    {row.targetingType || '-'}
                </span>
