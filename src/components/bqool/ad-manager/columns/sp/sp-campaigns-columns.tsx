...
import { GearFill, ChevronRight, CaretUpFill, CaretDownFill, Check, X, ChevronDown, InfoCircleFill } from "react-bootstrap-icons";
...
export const getSPCampaignColumns = (
    ...
    return [
                ...
                <div style={{ width: WIDTH_NAME }}>
                    <EntityCell 
                        ...
                        badges={[
                            ...
                            <Badge key="type" variant="type">{row.type || 'SP'}</Badge>
                        ]}
                    />
                </div>  
            ),
        ...
        { 
            ...
            render: (row) => (
                <div style={{ width: '180px' }}>
                    <EntityCell 
                        title={row.goalName || '-'} 
                        badges={row.goalStatus ? [
                            <Badge key="status" variant={row.goalStatus === 'Advanced' ? 'status-enabled' : 'type'}>
                                {row.goalStatus}
                            </Badge>
                        ] : []}
                    />
        ...
        { 
            key: 'portfolio', header: 'Portfolio', width: '140px',
            render: (row) => (
                <div style={{ width: '140px' }} className="text-sm text-gray-700 leading-tight line-clamp-2" title={row.portfolio}>
                    {row.portfolio || '-'}
                </div>
            )
        },
        {
            ...
            header: () => (
                <ColumnSorter
                    columnKey="budget" // Make sure data has 'budget' property
                    label="Daily Budget"
                    currentSort={currentSort}
                    onSort={onSort}
