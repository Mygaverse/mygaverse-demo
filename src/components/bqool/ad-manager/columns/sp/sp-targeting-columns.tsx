...
import { LightningFill, ChevronDown, ChevronRight, Check, X, CaretUpFill, CaretDownFill, InfoCircleFill } from "react-bootstrap-icons";
...
// ============================================================================
// MAIN COLUMN DEFINITION (SP)
// ============================================================================

export const getSPTargetingColumns = (
    ...
    return [
        ...
        { 
            ...
            render: (row) => (
                <div style={{ width: '300px' }}>
                    <EntityCell title={row.targetingText || 'Unknown Target'} badges={[
                        <Badge key="match" variant="neutral">{row.matchType || 'Broad'}</Badge>,
                        row.isAutoHarvested && (
                            <div key="harvest" className="flex items-center gap-1 text-[10px] text-[#0066b7] bg-[#f0f9ff] border border-[#0066b7] rounded-full px-2 py-0.5">
        ...
        { 
            ...
            width: '220px',
            render: (row) => <div style={{ width: '220px' }}><EntityCell title={row.goalName || '-'} badges={[<Badge key="g" variant="status-enabled">{row.goalStatus || 'Basic'}</Badge>]} /></div>
        },
        { 
            ...
            render: (row) => (
                <BiddingCardCell 
                    ...
                    onUpdateMinMax={(min, max) => {
                        onUpdateBid(row.id, 'minBid', min);
                        onUpdateBid(row.id, 'maxBid', max);
                    }}
                />
            )
        },
