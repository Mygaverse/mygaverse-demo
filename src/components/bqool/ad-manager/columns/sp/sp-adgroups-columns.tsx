...
import { GearFill, CaretUpFill, CaretDownFill, Check, X, ChevronDown, InfoCircleFill, ChevronRight, Pencil } from "react-bootstrap-icons";
...
const AdGroupBulkActionsMenu = ({ count, onAction }: BulkActionsMenuProps) => {
    ...
    const handleBidSubmit = (e: React.MouseEvent) => {
        e.stopPropagation();
        if (!bidVal) return;
        onAction('default-bid', parseFloat(bidVal));
        setIsOpen(false);
    };
    ...
    return (
        <>
            ...
            {isOpen && (
                <Portal>
                    <div 
                        className="fixed z-[9999] bg-white border border-gray-200 rounded-md shadow-xl py-1 w-64 animate-in fade-in zoom-in-95 duration-75"
                        style={{ top: coords.top, left: coords.left }}
                        onClick={(e) => e.stopPropagation()} 
                    >
                        {/* 1. Status Actions */}
                        <button onClick={() => { onAction('enable'); setIsOpen(false); }} className="block w-full px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 text-left">Set Status as Enabled</button>
                        <button onClick={() => { onAction('pause'); setIsOpen(false); }} className="block w-full px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 text-left">Set Status as Paused</button>
                        <button onClick={() => { onAction('archive'); setIsOpen(false); }} className="block w-full px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 text-left border-b border-gray-100">Set Status as Archived</button>
...
// ============================================================================
export const getSPAdGroupColumns = (
...
): ColumnDef<UnifiedAdGroup>[] => {

    const WIDTH_SELECT = '50px'; const WIDTH_ACTION = '50px'; const WIDTH_STATUS = '80px';
    const WIDTH_NAME = '220px'; const WIDTH_CAMP = '260px'; const WIDTH_GOAL = '200px'; const WIDTH_AI = '300px';
    ...
    return [
        { 
            ...
            width: WIDTH_SELECT, sticky: 'left', stickyOffset: '0px', align: 'center',
            render: (row, context) => (
        ...
        { 
            key: 'status', header: 'Status', width: WIDTH_STATUS, sticky: 'left', stickyOffset: '50px', align: 'center',
        ...
        { 
            ...
            sticky: 'left', stickyOffset: '200px', width: WIDTH_NAME,
            render: (row) => <div style={{ width: WIDTH_NAME }}><EntityCell title={row.name || 'Unknown'} /></div>,
            ...
            render: (row) => (
                <div style={{ width: WIDTH_CAMP }}>
                    <EntityCell title={row.campaignName || '-'} badges={[
                        <div key="s" className="flex items-center gap-1 bg-white border border-[#e2e2e2] rounded-full px-2 py-0.5"><img src={`https://flagcdn.com/w20/${row.flag || 'us'}.png`} className="w-3.5 h-2.5 rounded-[1px]"/><span className="text-[10px] text-gray-600">{row.storeName}</span></div>,
                        <Badge key="t" variant="neutral" size="sm">SP</Badge>
                    ]} />
                </div>
            )
        },
        { 
            ...
            width: WIDTH_GOAL,
            render: (row) => <div style={{ width: WIDTH_GOAL }}><EntityCell title={row.goalName || '-'} badges={[<Badge key="g" variant={row.goalType === 'Advanced' ? 'status-enabled' : 'neutral'}>{row.goalType || 'Basic'}</Badge>]} /></div>
        },
        { 
            key: 'defaultBid', header: 'Default Bids', align: 'center', width: '130px',
            render: (row) => <BudgetCell value={row.defaultBid ?? 0} isAuto={false} onChange={(val) => onBidChange(row.id, val)} />
        },
        {
        ...
        {
            key: 'autoHarvesting', header: 'Auto-Harvesting', width: '130px', align: 'center',
            render: (row) => <div className="flex justify-center"><Toggle checked={row.autoHarvestingEnabled ?? false} onChange={(val) => onUpdateAI(row.id, 'autoHarvestingEnabled', val)} /></div>
        },
        // Metrics
        { 
            key: 'sales', 
