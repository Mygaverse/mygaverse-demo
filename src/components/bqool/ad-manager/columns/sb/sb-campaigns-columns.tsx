...
export const getSBCampaignColumns = (
    ...
    const WIDTH_PORT   = '140px';
    const WIDTH_COST   = '100px';
    ...
    return [
                ...
                <div style={{ width: WIDTH_NAME }}>
                    <EntityCell 
                        ...
                        badges={[
                            ...
                            <Badge key="type" variant="type">{row.type || 'SB'}</Badge>
                        ]}
                    />
                </div>  
            ),
        ...
        // --- 5. PORTFOLIO ---
        { 
            key: 'portfolio', header: 'Portfolio', width: WIDTH_PORT,
            render: (row) => <div style={{ width: WIDTH_PORT }} className="text-sm text-gray-700 truncate" title={row.portfolio}>{row.portfolio || '-'}</div>
        },

        // --- 6. COST TYPE (CPC / vCPM) ---
        {
            key: 'costType', header: 'Cost Type', width: WIDTH_COST, align: 'center',
            render: (row) => (
                <span className="inline-block border border-gray-300 rounded-full px-2.5 py-0.5 text-[11px] font-medium text-gray-600 bg-white">
                    {row.costType || 'CPC'}
                </span>
            )
        },

        // --- 7. BUDGET (With Badges & Spent Info) ---
        { 
            key: 'budget', 
            header: 'Budget', 
            align: 'center', 
            ...
            render: (row) => {
                ...
                const spentPercent = budget > 0 ? (spend / budget) * 100 : 0;
                ...
                return (
                    <BudgetCell 
                        ...
                        // Detailed Info
                        budgetType={row.budgetType || 'Daily'} 
                        spent={parseFloat(spentPercent.toFixed(2))}
                        spentAmount={spend}
                    />
                );
            }
