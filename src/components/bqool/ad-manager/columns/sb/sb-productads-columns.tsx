...
import { StatusCell } from "../../../tables/cells/StatusCell"; // Assuming you have this or use CustomStatusCell
import { UnifiedProductAd } from "../../data/unifiedAdManagerData";
import { ColumnSorter, SortDirection } from "../../../ui/ColumnSorter";

// --- CUSTOM STATUS CELL ---
// Reusing logic: Handles enabled/paused/archived states gracefully
...
export const getSBProductAdsColumns = (
    ...
    const WIDTH_SELECT = '50px';  const WIDTH_STATUS = '80px';
    const WIDTH_PROD   = '380px'; const WIDTH_ADGRP  = '200px'; const WIDTH_CAMP   = '280px';
    const WIDTH_FORMAT = '140px';
    ...
    return [
        ...
        { 
            ...
            sticky: 'left', stickyOffset: '130px', width: WIDTH_PROD,
            render: (row) => (
                <div style={{ width: WIDTH_PROD }} className="flex items-center h-full">
                    {/* If no ad text, assume empty/error state like "No Product Ad" badge */}
                    {row.sbAdText ? (
                        <span className="text-sm text-gray-900">{row.sbAdText}</span>
                    ) : (
        ...
        // --- AD FORMAT (SB Specific) ---
        {
            ...
            render: (row) => (
                <span className="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-medium bg-gray-100 text-gray-700 border border-gray-200 whitespace-nowrap">
                    {row.adFormat || 'Product Collection'} 
                </span>
            )
        },
        // --- AD GROUP (With Status Badge) ---
        { 
            key: 'adGroup', 
            header: () => (
                <ColumnSorter
            ...
            width: WIDTH_ADGRP,
            render: (row) => (
                <div style={{ width: WIDTH_ADGRP }}>
                    <EntityCell 
                        title={row.adGroupName || '-'} 
                        badges={[
                            // Badge controlled by parent status (populated data), but functionally unrelated to row toggle
        ...
        // --- CAMPAIGN (With Store & Status Badge) ---
        { 
            key: 'campaign', 
            header: () => (
                <ColumnSorter
                    ...
                    <EntityCell 
                        ...
                        badges={[
                            <div key="store" className="flex items-center gap-1 bg-white border border-[#e2e2e2] rounded-full px-2 py-0.5">
                                <img src={`https://flagcdn.com/w20/${row.flag || 'us'}.png`} className="w-3.5 h-2.5 rounded-[1px]"/>
                                <span className="text-[10px] text-gray-600">{row.storeName}</span>
                            </div>,
                            <Badge key="type" variant="neutral" size="sm">SB</Badge>,
